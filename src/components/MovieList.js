...
const MovieList =({tab, setTab})=>{
    // const [tab, setTab] = useState( [
    //     { title:"Orgueil et préjugés", description:"Mrs. Bennet veut marier ses filles afin de leur assurer un avenir serein. L'arrivée de nouveaux voisins, Mr. Bingley et son ami Mr. Darcy, plonge Jane et Elisabeth dans des affaires de coeur tumultueuses.", posterURL:"https://fr.web.img6.acsta.net/medias/nmedia/18/35/99/80/18461215.jpg", rating:"2",id: Math.random() },
    ...
    //     { title:"Matrix 4 ", description:"Pour savoir si sa réalité est une construction physique ou mentale, M. Anderson, alias Neo, devra choisir de suivre une fois de plus le lapin blanc. ", posterURL:"https://fr.web.img4.acsta.net/pictures/21/11/17/17/24/3336846.jpg", rating:"2",id: Math.random() }
    //     // { title:"", description:"", posterURL:"", rating:"5",id: Math.random() },
    
    // ])
    const [rateF,setRateF] = useState(0)
    const [textSearch,setTextSearch]= useState("")
    const x =  tab.filter(movie => movie.title.toUpperCase().includes(textSearch.toUpperCase()) && movie.rating >= rateF).map(el=> <MovieCardTest tab={tab} setTab={setTab} el={el}/>)
    return(
        <div>
        ...
        <FilterMovie texSearch={textSearch} setTextSearch={setTextSearch} rateF={rateF} setRateF={setRateF} tab={tab} setTab={setTab}/>
        <br/>
        <AddModal tab={tab} setTab={setTab}/>
